...
void SøkBok()
{
    ...
    foreach (var b in resultater)
    {
        ...
        foreach (var l in aktive)
            Console.WriteLine($"   - {l.LånId}: lånt av {l.Låntaker} {l.Lånedato}");

        Console.WriteLine("  Historikk:");
        foreach (var l in historikk)
            Console.WriteLine($"   - {l.LånId}: returnert {l.ReturnertDato}");
    }
}
void LånBok()
{
    ...
    foreach (var lån in lånListe.Where(l => l.Aktiv))
        Console.WriteLine($"- {lån.LånId}: {lån.Bok.Tittel} lånt av {lån.Låntaker}");
}
void ReturnerBok()
{
    ...
    var lån = lånListe.FirstOrDefault(l => l.LånId == id && l.Aktiv);
    if (lån == null)
    {
    ...
    lån.ReturnertDato = DateTime.Now;
    lån.Bok.Antall++;

    Console.WriteLine("Bok returnert.");

    Console.WriteLine("\nLånehistorikk:");
    foreach (var h in lånListe.Where(l => !l.Aktiv))
        Console.WriteLine($"- {h.LånId}: {h.Bok.Tittel} returnert {h.ReturnertDato}");
}
